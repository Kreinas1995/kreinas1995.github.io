...
function watchResults(){
  ...
  new MutationObserver(()=>{
    ...
    if(holeCard&&playerCard&&!handActive) handActive=true;

    // Record result
    const wlWrap=document.querySelector('.win-lose-wrap');
    if(!wlWrap||!wlWrap.classList.contains('bj-show')) return;
    if(!handActive) return;
    handActive=false; // consume - prevents double fire

    // Snap balance NOW (after all bets deducted including double/split)
    // and compare to pre-hand balance from user-money data-money
    const balNow=getBankroll();
    const bet=getCurrentBet()||sess.lastBet||0;
    if(bet) sess.lastBet=bet;

    // Get pre-hand balance from the stored value
    // balNow already reflects post-result state
    // Use bet header amount as the total wagered (includes doubles)
    ...
    const wonEl=document.querySelector('.win-lose .wl-info .bj-wonState');
    const wonText=wonEl?.textContent||'';
    const amtMatch=wonText.match(/\$([0-9,]+)/);
    const amtWon=amtMatch?parseInt(amtMatch[1].replace(/,/g,'')):0;
    ...
    const isLoss=!isSurrender&&(wlClass.includes('lost')||msg.includes('lost')||msg.includes('lose')||msg.includes('bust'));
    const isPush=!isWin&&!isSurrender&&!isLoss;

    // Profit = amount won minus amount wagered (bet = total wagered including double)
    let profit;
    if(isWin)        profit=amtWon-bet;
    else if(isSurrender) profit=-(bet/2);
    else if(isLoss)  profit=-bet;
    else             profit=0; // push

    if(isWin)       sess.w=(sess.w||0)+1;
    else if(isLoss||isSurrender) sess.l=(sess.l||0)+1;
    else            sess.p=(sess.p||0)+1;
    sess.profit=(sess.profit||0)+profit;

    life.w=(life.w||0)+(isWin?1:0);
    life.l=(life.l||0)+((isLoss||isSurrender)?1:0);
