...
**Version:** 2.41.12  
**File:** `FactionBudget/current/FactionBudgetv2_41_12.html`  
**Total lines:** ~8,015
...
| `fetchInvPurchases()` | 5007 | Async. Fetches purchase history for museum items via user/log (multi-category: 86=Shops, 18=Bazaars, 11=Item market, 87=Travel, 88=Auctions, 94=Trades). Resolves numeric item IDs from `data.item` / `data.items[]` via `S.prices` catalog. PATH A: single-item entries (Shops/Travel) use `data.item` + `data.quantity`. PATH B: multi-item entries (Bazaar/Market/Trades) iterate `data.items[]` of `{id, qty}`. Detects losses via exact title match ("Bazaar add", "Item market add", "Trade items add", "Trade items outgoing"). Merges into `S.invPurchases` / `S.invSells`, deduped by log entry ID. |
...
### 8.8 user/log Structured Data vs Narrative Text
- The v1 user/log API returns **structured `data` fields with numeric item IDs**, NOT human-readable narrative strings.
- `entry.log` is an **integer log-type code** (e.g. `4200`, `4482`), not a description string. Narrative regex on `entry.log` is always dead code.
- `entry.title` is the category label (e.g. `"Item shop buy"`, `"Bazaar add"`). Use exact lowercase title matching, not regex.
- `entry.category` is the category group (e.g. `"Shops"`, `"Bazaars"`, `"Trades"`).
- Item data shapes by context:
  - **Shops / Travel** (cat 86/87): `data.item` = numeric item ID, `data.quantity` = qty
  - **Item market / Bazaar / Trades** (cat 11/18/94): `data.items` = `[{id, uid, qty}, ...]`
- To resolve numeric item ID → name: look up `S.prices[String(id)].name` (S.prices is keyed by string ID).
- **Never** use `String(data.item)` as an item name — it produces `"186"` which matches nothing.

### 8.9 Inventory Delta Persistence Bug Pattern
- `clearSessionState()` must NOT wipe `S.invPurchases` or `S.invSells`. These are persisted inventory data, not ephemeral sync cache. Wiping them in-memory causes `calcInvDelta()` to return zero even when `hv_v1_inv_purchases` in localStorage has valid data.
- Only `adminClearAll()` (full wipe) should clear purchase/sell history.
- General rule: anything persisted to its own localStorage key that survives a sync reset should NOT be in `clearSessionState()`.
